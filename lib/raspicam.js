...
RaspiCam.prototype.capture = function (eventdatetime) {
    ...
    var neweventstr = './video/events/' + eventdatetime + '.h264';
    var fs = require('fs');
    ...
    fs.createReadStream('./video/video.h264').pipe(fs.createWriteStream(neweventstr));
    PROCESS_RUNNING_FLAG = false;
    //var fs = require('fs');
...
RaspiCam.prototype.start = function () {
    ...
    switch (this.opts.mode) {
        case 'video':
            var newargs = ['-c', '-vf', '-hf', '-o', './video/video.h264', '-k', '-t', '15000' , '-b' , '2000000']// ,'-v']
            console.log(newargs);
