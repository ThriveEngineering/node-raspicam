...
RaspiCam.prototype.watchDirectory = function () {
    ...
    this.watcher = fs.watch(this.filepath, function (event, filename) {
        ...
        if (event === "rename") {
            ...
            console.log('raspicam::watcher::event2 ' + event);
            self.emit(event, null, new Date().getTime(), filename);
        }
    });
};

RaspiCam.prototype.switchclose = function () {
    var self = this;
    ...
    child_process = null;
    this.watcher = null;
    console.log('in swtichclose');
    console.log(PROCESS_RUNNING_FLAG);
    self.emit("exit", new Date().getTime());
}

RaspiCam.prototype.capture = function (eventdatetime) {
    console.log('capturing');
    var neweventstr = '/opt/thrive/video/events/' + eventdatetime + '.h264';
    var fs = require('fs');
    ...
    ensureDirectoryExistence(neweventstr);
    fs.createReadStream('/opt/thrive/video/video.h264').pipe(fs.createWriteStream(neweventstr));
    PROCESS_RUNNING_FLAG = false;
    //var fs = require('fs');
    //var file = fs.createWriteStream('./video/video1.h264');
    //console.log(childprocess.stdout);
    //childprocess.stdout.pipe(file);
    //file.end();
    //child_process.kill();
}
/**
...
RaspiCam.prototype.start = function () {
    ...
    switch (this.opts.mode) {
        ...
        case 'timelapse':
            ...
            // if not timeout provided, set to longest possibled
            if (typeof this.opts.timeout === "undefined") {
    ...
    switch (this.opts.mode) {
        case 'video':
            var newargs = ['-c', '-o', '/opt/thrive/video/video.h264', '-k', '-t', '15000' , '-b' , '1000000']// ,'-v']
            console.log(newargs);
            this.child_process = spawn(cmd, newargs);
            break;
        default:
    ...
    //console.log('now setting up child listeners...');
...
RaspiCam.prototype.addChildProcessListeners = function () {
    ...
    this.child_process.stderr.on('data', function (data) {
        ...
        if (data.indexOf('nished capture 0') > 0) {
            //Then we need to stop this process...
            console.log('stop!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!');
            self.emit("exit", new Date().getTime());
            PROCESS_RUNNING_FLAG = false;
            self.child_process = null;
